...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, PublicNavComponent, CodeBlockComponent, ButtonComponent, AppFooterComponent],
  template: `
    <app-public-nav />
    <div class="mx-auto max-w-7xl px-4 sm:px-6 pt-20 sm:pt-24 pb-12 flex flex-col lg:flex-row gap-6 lg:gap-10">
      ...
      <div class="lg:hidden sticky top-14 sm:top-16 z-40 -mx-4 sm:-mx-6 px-4 sm:px-6 py-3 bg-ink-950/95 backdrop-blur border-b border-ink-800">
        ...
        <nav class="sticky top-24 space-y-1 text-sm">
          @for (s of sections; track s.id) {
            <button (click)="active = s.id" class="block w-full text-left rounded-lg px-3 py-2.5 transition-colors"
              [class]="active === s.id ? 'bg-signal-500/15 text-signal-400 font-medium border-l-2 border-signal-500' : 'text-ink-500 hover:text-white hover:bg-ink-800/50'">
              {{ s.title }}
            </button>
          }
        </nav>
      ...
      <main class="flex-1 min-w-0 pb-20">
        @if (active === 'concepts') {
          ...
          <p class="mt-4 text-ink-500 leading-relaxed max-w-3xl">
          ...
          <div class="mt-8 grid gap-4 md:grid-cols-2">
            <div class="card border-signal-500/20">
              <h3 class="font-semibold text-signal-400">Your users</h3>
              <p class="mt-2 text-sm text-ink-500">Use your app normally. They never sign up for ChainProof.</p>
            </div>
            <div class="card border-signal-500/20">
              <h3 class="font-semibold text-signal-400">Your backend</h3>
              <p class="mt-2 text-sm text-ink-500">Calls <code class="text-signal-400">/integrity/anchor</code> after each save using your ChainProof API key.</p>
            </div>
          </div>
          <div class="mt-8 card">
            ...
            <ol class="space-y-2 text-sm text-slate-300 list-decimal list-inside">
              ...
              <li>After each write in <em>your</em> database, POST <code class="text-signal-400">/integrity/anchor</code> with <code class="text-signal-400">entity_id</code>, <code class="text-signal-400">payload</code>, and <code class="text-signal-400">verify</code>.</li>
              <li>The <code class="text-signal-400">verify</code> block tells ChainProof how to re-fetch that exact record later (any HTTP path, any <code class="text-slate-300">{{ '{' }}param{{ '}' }}</code>).</li>
        ...
        @if (active === 'developer') {
          ...
          <p class="mt-3 text-ink-500 max-w-3xl leading-relaxed">
          ...
          <ol class="mt-2 text-sm text-ink-500 space-y-1 list-decimal list-inside">
            <li><a routerLink="/register" class="text-signal-400 hover:underline">Register</a> → Dashboard → <strong class="text-white">Sites</strong> → Add your backend URL.</li>
            <li>Select the site → copy <strong class="text-white">Site ID</strong> (UUID shown on the site card).</li>
            <li>Dashboard → <strong class="text-white">API Keys</strong> → Generate → copy once into your server <code class="text-signal-400">.env</code>.</li>
          </ol>
          ...
          <p class="text-sm text-ink-500 mt-2">
            <code class="text-signal-400">payload</code> must match what you would return from your read API.
            <code class="text-signal-400">verify</code> tells ChainProof how to re-fetch it (supports path params like <code class="text-slate-300">/orders/{{ '{' }}order_id{{ '}' }}</code>).
          </p>
          ...
          <div class="mt-6 grid gap-3 md:grid-cols-2 lg:grid-cols-4 text-sm">
            <div class="card"><strong class="text-white">site_id</strong><p class="text-ink-500 mt-1">From Dashboard → Sites → select site → Site ID</p></div>
            <div class="card"><strong class="text-white">entity_type</strong><p class="text-ink-500 mt-1">Your resource name: payment_transaction, payroll_record, employment_contract</p></div>
            <div class="card"><strong class="text-white">entity_id</strong><p class="text-ink-500 mt-1">Stable record ID (UUID, order #, etc.)</p></div>
            <div class="card"><strong class="text-white">verify</strong><p class="text-ink-500 mt-1">How ChainProof re-fetches this record for tamper checks</p></div>
          </div>

          <h2 class="mt-8 text-lg font-semibold text-white">Step 3 — Path params (dynamic IDs)</h2>
          <p class="text-sm text-ink-500 mt-2">Use placeholders in <code class="text-signal-400">path_template</code>. Resolve them in <code class="text-signal-400">path_params</code>:</p>
          <ul class="mt-2 text-sm text-ink-500 list-disc list-inside space-y-1">
            <li><code class="text-signal-400">$entity_id</code> — uses the anchor <code class="text-slate-300">entity_id</code></li>
            <li><code class="text-signal-400">$payload.field</code> — reads a field from the anchored payload</li>
          ...
          <p class="text-sm text-ink-500 mt-2">
            ...
            Routes with <code class="text-slate-300">{{ '{' }}id{{ '}' }}</code> should use the anchor <code class="text-signal-400">verify</code> block instead.
          </p>

          <div class="mt-8 card border-signal-500/20">
            <p class="text-sm text-slate-300"><strong class="text-white">Auth:</strong> Use <code class="text-signal-400">X-API-Key: cp_...</code> or <code class="text-signal-400">Authorization: Bearer cp_...</code>. ChainProof never stores your users' JWTs.</p>
...
export class DocsPageComponent {
  ...
  "entity_id": "txn_8f2a91c4-b12e-4d9a-9f03-2c7e5a1d8b6f",
  "site_id": "YOUR_SITE_UUID",
  "payload": {
    "id": "txn_8f2a91c4-b12e-4d9a-9f03-2c7e5a1d8b6f",
    "amount": 149.99,
    "currency": "USD",
    "status": "completed",
  ...
  "verify": {
    "method": "GET",
    "path_template": "/api/v1/payments/{transaction_id}",
    "path_params": { "transaction_id": "$entity_id" },
  ...
  get samples() {
    ...
    body, _ := json.Marshal(map[string]interface{}{
        ...
        "verify": map[string]interface{}{
            "method":        "GET",
            "path_template": "/api/v1/payments/{transaction_id}",
            "path_params":   map[string]string{"transaction_id": "$entity_id"},
            "payload_from":  "response",
        },
    })
  ...
  await fetch(\`\${process.env.CHAINPROOF_BASE_URL}/integrity/anchor\`, {
    ...
    var body = Map.of(
        ...
        "verify", Map.of(
            "method", "GET",
            "path_template", "/api/v1/payments/{transaction_id}",
            "path_params", Map.of("transaction_id", "$entity_id"),
            "payload_from", "response"
        )
    );
