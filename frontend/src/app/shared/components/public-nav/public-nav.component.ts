    ...
    <nav class="fixed top-0 z-50 w-full border-b border-slate-800/60 bg-slate-950/90 backdrop-blur-xl">
      <div class="mx-auto flex h-14 sm:h-16 max-w-7xl items-center justify-between gap-3 px-4 sm:px-6">
        <a routerLink="/" class="flex min-w-0 items-center gap-2 sm:gap-3" (click)="closeMobile()">
          <div class="flex h-8 w-8 sm:h-9 sm:w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-brand-500 to-emerald-500 text-xs sm:text-sm font-bold shadow-lg shadow-brand-600/30">CP</div>
        ...
        <div class="hidden md:flex items-center gap-3 shrink-0">
          @if (auth.isLoggedIn()) {
            <span class="hidden lg:inline text-xs text-slate-500 truncate max-w-[140px]">{{ auth.user()?.email }}</span>
        ...
        <!-- Mobile menu toggle -->
        <button type="button" class="md:hidden inline-flex items-center justify-center rounded-lg p-2 text-slate-300 hover:bg-slate-800 hover:text-white"
