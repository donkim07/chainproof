...
@Component({
  ...
  template: `
    ...
    <div class="grid gap-4 sm:grid-cols-3 mb-8">
      <app-stat-card label="Tools available" [value]="availableCount + '/' + (status?.tools?.length ?? 0)" color="text-signal-400" icon="&#128269;"></app-stat-card>
      <app-stat-card label="Wordlist" [value]="status?.wordlist_ok ? 'Ready' : 'Missing'" [color]="status?.wordlist_ok ? 'text-signal-400' : 'text-alert-400'" icon="&#128196;"></app-stat-card>
    ...
    <div class="card mb-6">
      ...
      <div class="grid gap-3 md:grid-cols-3">
        @for (tool of status?.tools ?? []; track tool.name) {
          <div class="rounded-xl border p-4 transition-all hover:border-signal-500/30"
            [class]="tool.available ? 'border-signal-500/30 bg-signal-500/5' : 'border-ink-700 bg-ink-900/50'">
            ...
            @if (tool.version) {
              <p class="mt-2 text-xs text-ink-500 truncate">{{ tool.version }}</p>
            }
            @if (tool.path) {
              <p class="mt-1 text-xs font-mono text-ink-700 truncate">{{ tool.path }}</p>
            }
          </div>
        }
      </div>
    ...
    <div class="card">
      ...
      <ol class="space-y-3 text-sm text-ink-500 list-decimal list-inside">
      ...
      <p class="mt-4 text-xs text-ink-500 font-mono">Wordlist: {{ status?.wordlist_path }}</p>
    </div>
  `,
})
